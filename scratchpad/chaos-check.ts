 ...
 *
 *   node scripts/shot.mjs scratchpad/chaos-check.html out.png 1900 1300
 *   ?set=phases    the 6 kinds × 4 phases contact sheet (default)
 *   ?set=spread    the pocket's own vine/frond multiplicity, to judge variance
 *
 * Static: renders once the models land, then sets __ready. Same lighting as
...
/** Extra clear samples, in seconds after playClear(). */
...
function buildCells() {
if (set === 'phases') {
  const phases: { label: string; strain: number; clear: number }[] = [
  ...
  // Variance: the five vines and four fronds the pocket actually places, at the
  // variant numbers it actually passes.
...
/*
 * Cells are built AFTER loadModels() resolves, not at module scope.
 *
 * A rig created while the cache is still cold registers its own
 * loadModels().then(attach), and loadModels does not memoise its in-flight
 * promise — so the page's load and the rigs' loads are separate chains that
 * resolve in arbitrary order, and a draw fired off the page's chain caught some
 * props before their mesh had been attached. The game never sees this (boot
 * warms the cache long before beat 5 builds the pocket), but the harness has to
 * be told.
 */
void loadModels().then(() => {
  ...
  start(cols, cellPx, rows)
  draw(cols, cellPx, rows)
...
// preserveDrawingBuffer: the page draws once and stops, and a CDP screenshot
// of a swapped-away WebGL buffer is a blank rectangle.
...
// Drawn once, and only after the glTF lands: `drive()` advances real rig state,
// so a second pass would push every clearing cell twice as far through its
// animation as the label claims.
void loadModels().then(() => {
  draw()
  ;(window as unknown as { __ready?: boolean }).__ready = true
})
